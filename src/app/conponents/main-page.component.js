...
class MainPageComponent extends Component {
  ...
  setImg(url) {
    ...
    imageContainer.style.backgroundImage = `url(${API_URL}${url})`;
  }
  ...
  playMeaningExampleSound({ target }) {
    ...
    this.isPlay = true;
    this.audio = new Audio(`${API_URL}${span.dataset.audio}`);
  ...
  async wordSound({ target }) {
    ...
    const audio = new Audio(`${API_URL}${input.dataset.audio}`);
    ...
    translate.value = input.dataset.wordTranslate;
...
export const appMainPage = new MainPageComponent({
  selector: '#main-page',
  template: require('../templates/main-page.html'),
});
